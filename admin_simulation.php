...
{	
	...
	document.getElementById("myTime").stepDown(x);
}

function minutesUP() 
{
	var x=document.getElementById("myNumber").value;	
	document.getElementById("myTime").stepUp(x);
}
...
for(var j = 0; j <=id_len; j++) 
{
	...
	panagia[j]={
		"type": "Feature",
		"properties": { "id": j,
						"percentOccupied":parseInt((100*((parseInt(parking[j].positions)-parseInt(20/100 * population[j].population))*temp_curve[what_time]))/parking[j].positions),
